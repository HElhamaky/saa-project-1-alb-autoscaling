...
`terraform apply` completed with **65 added, 0 changed, 0 destroyed** and no
warnings.

> **Reconciling the resource counts.** The stack was deployed twice. The first
> run created **63** resources. The SNS notification fix then added a
> customer-managed KMS key and its alias, so the second run created **65**.
> `terraform state list` reports **69**, because state also tracks the **4 data
> sources** (`aws_availability_zones`, `aws_caller_identity`, `aws_region`,
> `aws_ssm_parameter`) - read-only lookups Terraform resolves rather than
> creates. So 65 managed resources + 4 data sources = 69 in state, and
> `01-terraform-apply-complete.png` showing 69 is the same stack as the 65 in
> this document.
