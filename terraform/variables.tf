...
variable "db_engine_version" {
  description = <<-EOT
    MySQL major version. Keep this on a version in STANDARD support.

    Running a major version past its standard-support end date puts the
    instance into RDS Extended Support, billed per vCPU-hour on top of the
    instance itself. In this project's first run the database was left on 8.0
    after its standard support ended, and the surcharge came to $8.86 against
    $0.75 of actual instance cost - 72% of the entire bill for the whole
    stack. Check with:

      aws rds describe-db-engine-versions --engine mysql --default-only

    The default AWS reports there is the version currently in standard
    support.
  EOT
  type        = string
  default     = "8.4"
