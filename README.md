...
Author: Hassan Samy · Submitted: 7 September 2026
...
└── docs/
    ...
    ├── DIAGRAM-SPEC.md
    ├── EVIDENCE-CAPTURE.md       step-by-step for all 15 screenshots
    ├── evidence-values.ps1       prints live endpoints and instance IDs
    ├── start-burn.ps1            drives the Auto Scaling demo
    ├── failover-evidence.ps1     before/after view of the RDS failover
...
## What I estimated before deploying
...
| **Estimated total** | | **~$4.10 / day** |

## What it actually cost

Pulled from Cost Explorer after teardown, filtered to `RECORD_TYPE = Usage`
so credits do not mask the real figure:

```bash
aws ce get-cost-and-usage --time-period Start=2026-08-27,End=2026-08-31 \
  --granularity MONTHLY --metrics UnblendedCost \
  --filter '{"Dimensions":{"Key":"RECORD_TYPE","Values":["Usage"]}}' \
  --group-by Type=DIMENSION,Key=SERVICE
```

| Service | Actual | Share |
|---|---|---|
| **Amazon RDS** | **$9.75** | **79%** |
| EC2 – Other (NAT Gateway, EBS) | $1.07 | 9% |
| Elastic Load Balancing | $0.50 | 4% |
| EC2 – Compute (the instances) | $0.47 | 4% |
| VPC (Elastic IP, endpoints) | $0.33 | 3% |
| WAF | $0.16 | 1% |
| Secrets Manager | $0.02 | — |
| KMS | $0.01 | — |
| **Total for the whole run** | **$12.31** | |

The stack ran from the evening of 27 August to the afternoon of 29 August —
roughly 45 hours, of which about 22 were at full two-AZ capacity.

**Net charged: $0.00.** The account carried promotional credits, and Cost
Explorer recorded `Credit −$12.31` against `Usage $12.31`. The credits are why
the default Cost Explorer view reads zero — a useful reminder that *net* cost
and *incurred* cost are different questions, and only the second one tells you
whether your architecture is affordable.

### Where the estimate was wrong, and why it matters

My estimate was **$4.10/day**; the real figure was **$12.31 for the run**.
Against ~45 hours that is roughly $6.50/day — about 60% over.

Splitting RDS by usage type explains all of it, and the answer was not what I
first assumed:

```bash
aws ce get-cost-and-usage --time-period Start=2026-08-27,End=2026-08-31 \
  --granularity MONTHLY --metrics UnblendedCost UsageQuantity \
  --filter file://rds-usage-filter.json \
  --group-by Type=DIMENSION,Key=USAGE_TYPE
```

| RDS usage type | Cost | Share of RDS |
|---|---|---|
| **`ExtendedSupport:Yr1-Yr2:MySQL8.0`** | **$8.86** | **91%** |
| `Multi-AZUsage:db.t3.micro` | $0.75 | 8% |
| `RDS:Multi-AZ-GP3-Storage` | $0.14 | 1% |
| `RDS:ChargedBackupUsage` | $0.00 | — |

**The database instances cost 75 cents. A support surcharge cost $8.86** —
twelve times the compute, and 72% of the entire bill for the whole stack.

#### What RDS Extended Support is

When a major engine version reaches the end of its **standard support** date,
AWS does not force an upgrade and does not shut the database down. It keeps
running and starts billing **Extended Support**, charged per vCPU-hour on top
of the instance cost.

MySQL 8.0 passed that date before this project was deployed. The instance was
`db.t3.micro` — 2 vCPUs — and Multi-AZ means two of them, so 4 vCPUs were
accruing the surcharge for every hour the stack existed: 88.6 vCPU-hours at
$0.10, or $8.86.

Nothing warned me. `terraform apply` succeeded, the database worked, and the
charge appeared only in the usage-type breakdown after teardown.

#### The fix: one line

```hcl
db_engine_version = "8.4"   # was "8.0"
```

RDS reports which version currently carries standard support:
...
AWS moves the *default* to the supported version — so the default the API
reports is the answer. Had the project used it, the run would have cost
**$3.45 instead of $12.31, a 72% reduction**, with an identical architecture.
The parameter group family has to move with it (`mysql8.0` -> `mysql8.4`);
`require_secure_transport` exists in both, so TLS enforcement is unaffected.

Both changes are now the defaults in this repository.

#### The lesson

I had assumed the overrun was Multi-AZ storage, and wrote that down before
checking. It was wrong. Grouping by `SERVICE` said "RDS is 79% of the bill",
which is true and useless — it points at a service, not a cause. Only
`USAGE_TYPE` revealed that 91% of the database cost had nothing to do with
the database's size, storage, or Multi-AZ setting at all.

**When a cost surprises you, group by usage type before forming a theory.**
Service-level totals tell you where to look; usage types tell you what is
actually happening. The same instinct as reading `NumberOfMessagesPublished`
rather than guessing why an email never arrived.

### The observation worth keeping

**The web servers cost $0.47.** The thing the project is nominally about —
serving pages — was the cheapest meaningful line item on the bill. Almost
everything I paid for was resilience, plumbing, and a version-support
surcharge: a standby database that served no traffic, a NAT Gateway that only
forwards packets, a load balancer that holds no state.

That ratio is the real lesson of this exercise. **Availability is not a
feature you add to compute; it is the overwhelming majority of what you buy** —
and a lifecycle detail you never chose can outweigh every sizing decision you
did make.
